...
class twoStreamClassification(nn.Module):
    def __init__(self, num_class, Npole, Drr, Dtheta, data_dim, gpu_id, gumbel_inference_mode,
                  nClip, fistaLam, dataType, kinetics_pretrain,fineTune, useCL):
        ...
        self.dynamicsClassifier = contrastiveNet(Npole=self.Npole, Drr=self.Drr, Dtheta=self.Dtheta, gumbel_inference_mode=self.Inference, gpu_id=self.gpu_id, data_dim=self.data_dim,backbone_mode='DIR', 
                                                 dataType=self.dataType, fistaLam=self.fistaLam, fineTune=fineTune, nClip=self.nClip, useCL=useCL)
        
        self.RGBClassifier = contrastiveNet(Npole=self.Npole, Drr=self.Drr, Dtheta=self.Dtheta, gumbel_inference_mode=self.Inference, gpu_id=self.gpu_id, data_dim=self.data_dim,backbone_mode='rgb', 
                                                 dataType=self.dataType, fistaLam=self.fistaLam, fineTune=fineTune, nClip=self.nClip, useCL=useCL)
